...
#include <cutils/properties.h> // Samsung Bluetooth Feature.2011.11.21 - Apply IT Policy
...
// BEGIN SS_BLUEZ_BT  (+dykim 2011. 09. 09)
// To organize Qualcomm's 2nd patch codes, I make a flag.
#define ENABLE_QUALCOMM_SECOND_PATCH
// END SS_BLUEZ_BT
...
#if defined(FEATURE_DEV_I9100) && defined(FEATURE_OP_MEA_PCL)
				strcpy(value, "Pelephone Galaxy SII");
#elif defined(FEATURE_DEV_I9100) && defined(FEATURE_OP_MEA_PTR)
				strcpy(value, "Galaxy S II - orange");
#elif defined(FEATURE_DEV_I9100) && defined(FEATURE_OP_MEA_CEL)
				strcpy(value, "Cellcom Galaxy S II");
#endif
						...
						device_type_t type)
{
	struct btd_device *device;
	const char *path;
// BEGIN SS_BLUEZ_BT +dykim 2011.10.30.
// workaround for SMB-9200B (Samsung BT mouse)	
	gboolean gbParam = TRUE;
// END SS_BLUEZ_BT
#ifdef ENABLE_QUALCOMM_SECOND_PATCH
	bdaddr_t remote;
	uint32_t class = 0;
	char *iconstr = NULL;
#endif //#ifdef ENABLE_QUALCOMM_SECOND_PATCH

//	DBG("%s", address);   // log checker (BT)
...
// BEGIN SS_BLUEZ_BT +dykim 2011.10.30.
// workaround for SMB-9200B (Samsung BT mouse)
	if (!(strncmp(address, "00:12:a1", strlen("00:12:a1")) && strncmp(address, "00:12:A1", strlen("00:12:A1"))))
	{
								...
								void *data)
{
	...
	// DBG("Agent registered for hci%d at %s:%s", adapter->dev_id, name, path);
	DBG("Agent registered for hci%d at :", adapter->dev_id);   // log checker (BT)
...
//Samsung Bluetooth Feature - changes for MAP profile
...
static int add_opush_record(struct btd_adapter *adapter)
{
	...
	uint8_t u8 = 12;
	sdp_data_t *channel;
// Samsung Bluetooth Feature - (2012.01.09) Support all formats for opp
#if 0//def ANDROID
...
static void load_connections(struct btd_adapter *adapter)
{
	...
	for (l = conns; l != NULL; l = g_slist_next(l)) {
		...
		DBG("Adding existing connection to ");   // log checker (BT)
...
void adapter_remove(struct btd_adapter *adapter)
{
	...
	DBG("Removing adapter ");   // log checker (BT)
						...
						struct btd_device *device)
{
	...
	if (device_is_temporary(device)) {
		...
		DBG("Removing temporary device ");   // log checker (BT)
					...
					void *user_data)
{
	...
	struct agent *agent;
	char address[18];
	char value[PROPERTY_VALUE_MAX];	 // Samsung Bluetooth Feature.2011.11.21 - Apply IT Policy
	const gchar *dev_path;
	int err;
	...
	property_get("service.bt.security.policy.mode", value, "0"); // Samsung Bluetooth Feature.2011.11.21 - Apply IT Policy	 
	...
	// Samsung Bluetooth Feature.2011.11.21 - Apply IT Policy
