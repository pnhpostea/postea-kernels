...
void device_add_connection(struct btd_device *device, DBusConnection *conn)
{
	if (device->connected) {
		...
		error("Device %s is already connected", "xxxxxxxx"); //addr
		return;
	}
...
void device_remove_connection(struct btd_device *device, DBusConnection *conn)
{
	if (!device->connected) {
		char addr[18];
		ba2str(&device->bdaddr, addr);
		error("Device %s isn't connected", "xxxxxxxx"); //addr
		return;
	}
				...
				const gchar *address, device_type_t type)
{
	...
	DBG("Creating device " );   // log checker (BT)
...
static void device_remove_stored(struct btd_device *device)
{
	...
	/* SS_BLUETOOTH(jubyung.lee) 2012.02.01 */
	// keep alias name for the paired device
	delete_entry(&src, "aliases", addr);
	/* SS_BLUETOOTH(jubyung.lee) End */
...
void device_remove(struct btd_device *device, gboolean remove_stored)
{
	...
	DBG("Removing device ");   // log checker (BT)
...
void device_probe_drivers(struct btd_device *device, GSList *profiles)
{
	...
	if (device->blocked) {
		DBG("Skipping drivers for blocked device %s", "xxxxxxxx"); //addr
		goto add_uuids;
	}

	DBG("Probing drivers for %s", "xxxxxxxx"); //addr
...
static void device_remove_drivers(struct btd_device *device, GSList *uuids)
{
	...
	DBG("Removing drivers for %s", "xxxxxxxx"); //dstaddr
...
static void search_cb(sdp_list_t *recs, int err, gpointer user_data)
{
	...
	if (err < 0) {
		error("%s: error updating services: %s (%d)",
				"xxxxxxxx", strerror(-err), -err); //addr
		goto send_reply;
	}
	...
	if (!req->profiles_added && !req->profiles_removed) {
		DBG("%s: No service update", "xxxxxxxx"); //addr
		goto send_reply;
	}
						...
						uint8_t capability)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Requesting bonding for %s", "xxxxxxxx"); //addr
...
static void create_bond_req_exit(DBusConnection *conn, void *user_data)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("%s: requestor exited before bonding was completed", "xxxxxxxx"); //addr
...
void device_cancel_bonding(struct btd_device *device, uint8_t status)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Canceling bonding request for %s", "xxxxxxxx");  //addr
						...
						uint32_t passkey, void *cb)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Requesting agent authentication for %s", "xxxxxxxx");  //addr

	if (device->authr) {
		error("Authentication already requested for %s", "xxxxxxxx");  //addr
		return -EALREADY;
	}
...
void device_cancel_authentication(struct btd_device *device, gboolean aborted)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Canceling authentication request for %s", "xxxxxxxx");  //addr
