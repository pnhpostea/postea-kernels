...
//	DBG("State changed %s: %s -> %s", dev->path, str_state[old_state],
	DBG("State changed : %s -> %s", str_state[old_state],   // log checker (BT)
		str_state[new_state]);
...
//	DBG("Unregistered interface %s on path %s", AUDIO_SINK_INTERFACE, dev->path);
	DBG("Unregistered interface %s on path ", AUDIO_SINK_INTERFACE);   // log checker (BT)
...
//	DBG("Registered interface %s on path %s", AUDIO_SINK_INTERFACE, dev->path);
	DBG("Registered interface %s on path ", AUDIO_SINK_INTERFACE);   // log checker (BT)
