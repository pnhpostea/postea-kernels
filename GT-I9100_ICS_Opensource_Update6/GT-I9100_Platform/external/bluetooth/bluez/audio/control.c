				...
				uint16_t len)
{
	...
	if (write(sock, packet, len) < 0) {
		error("Can not write socket errno = %d!!", errno);
...
static void avrcp_supported_events_rsp(DBusPendingCall *call, void *user_data)
{
	...
	DBG("AVRCP_EVENTS_SUPPORTED RSP");
	dbus_error_init(&err);
						...
						void *user_data)
{
	...
	DBG("AVRCP_LIST_PLAYER_SETTING_ATTRS RSP");
	dbus_error_init(&err);
	if (dbus_set_error_from_message(&err, msg)) {
		if (g_str_equal(DBUS_ERROR_UNKNOWN_METHOD, err.name) ||
				g_str_equal(DBUS_ERROR_NO_REPLY, err.name)) {
						...
						void *user_data)
{
	...
	DBG("AVRCP_LIST_PLAYER_SETTING_VALS RSP");
	dbus_error_init(&err);
	if (dbus_set_error_from_message(&err, msg)) {
		if (g_str_equal(DBUS_ERROR_UNKNOWN_METHOD, err.name) ||
				g_str_equal(DBUS_ERROR_NO_REPLY, err.name)) {
						...
						void *user_data)
{
	...
	DBG("AVRCP_GET_PLAYER_SETTING_VAL RSP");
	dbus_error_init(&err);
	if (dbus_set_error_from_message(&err, msg)) {
		if (g_str_equal(DBUS_ERROR_UNKNOWN_METHOD, err.name) ||
				g_str_equal(DBUS_ERROR_NO_REPLY, err.name)) {
						...
						void *user_data)
{
	...
	DBG("AVRCP_SET_PLAYER_SETTING_VAL RSP");
	dbus_error_init(&err);
	if (dbus_set_error_from_message(&err, msg)) {
		if (g_str_equal(DBUS_ERROR_UNKNOWN_METHOD, err.name) ||
				g_str_equal(DBUS_ERROR_NO_REPLY, err.name)) {
...
static void avrcp_register_notification_rsp(DBusPendingCall *call, void *user_data)
{
	...
	case AVRCP_EVENT_PLAYBACK_STATUS_CHANGED:
		DBG("AVRCP_EVENT_PLAYBACK_STATUS_CHANGED RSP");
	...
	case AVRCP_EVENT_TRACK_CHANGED:
		DBG("AVRCP_EVENT_TRACK_CHANGED RSP");
		if (!dbus_message_get_args(msg, &err, DBUS_TYPE_BYTE, &event,
	...
	case AVRCP_EVENT_PLAYER_APPLICATION_SETTING_CHANGED:
		...
		break;
	default:
		error("Unknown/NotSupported Event Received");
		error_code = AVRCP_ERR_INVALID_PARM;
		goto error;
	}
						...
						struct playeragent *pa)
{
	...
	DBG("AVRCP_GET_EVENTS_SUPPORTED");
	...
	if (!dbus_message_append_args(msg,
					...
					&pa->control->dev->path,
					DBUS_TYPE_INVALID)) {
		error("Could not add argument to msg.");
		dbus_message_unref(msg);
		return AVRCP_REPLY_ERR;
	}
	...
	if (connection) {
		...
		error("DBUS connection problem");
		dbus_message_unref(msg);
		return AVRCP_REPLY_ERR;
	}

	if (!pa->call) {
		error( "Pending Call Null");
		dbus_message_unref(msg);
		return AVRCP_REPLY_ERR;
	}
					...
					struct playeragent *pa)
{
	...
	if (query->param_len != 1) {
		error("Wrong param len = %d", query->param_len);
		return AVRCP_REPLY_ERR;
	}
	...
	if ((attr_id == 0x01)||(attr_id == 0x04)) {
		DBG("List_player_setting_vals param id not supported");
				...
				struct playeragent *pa)
{
	...
	DBG("AVRCP_ELEMENT_ATTRIBUTES");
	if (identifier != 0x00) {
		error("Only Playing is supported");
		return AVRCP_REPLY_ERR;
	}
					...
					uint16_t packet_size)
{
	...
	case AVRCP_GET_CAPABILITIES:
		...
		default:/* Error: capability asked is not valid */
			error("capability asked is invalid 0x%02x",
							query->param[0]);
			status = AVRCP_REPLY_ERR;
...
static void avctp_disconnected(struct audio_device *dev)
{
	...
	if (control->uinput >= 0) {
		...
		DBG("AVRCP: closing uinput for ");   // log checker (BT)
...
//		error("AVRCP: failed to init uinput for %s", address);
		error("AVRCP: failed to init uinput for ");   // log checker (BT)
	else
//		DBG("AVRCP: uinput initialized for %s", address);
		DBG("AVRCP: uinput initialized for ");   // log checker (BT)
}
...
//	DBG("AVCTP: connected to %s", address);
	DBG("AVCTP: connected to ");   // log checker (BT)
...
//		error("Unable to get audio device object for %s", address);
		error("Unable to get audio device object for ");   // log checker (BT)
		goto drop;
	}
...
//		error("Refusing unexpected connect from %s", address);
		error("Refusing unexpected connect from ");   // log checker (BT)
		goto drop;
	}
...
static void path_unregister(void *data)
{
	...
	// DBG("Unregistered interface %s on path %s", AUDIO_CONTROL_INTERFACE, dev->path);
	DBG("Unregistered interface %s on path ", AUDIO_CONTROL_INTERFACE);   // log checker (BT)
...
struct control *control_init(struct audio_device *dev, uint16_t uuid16)
{
	...
	// DBG("Registered interface %s on path %s", AUDIO_CONTROL_INTERFACE, dev->path);
	DBG("Registered interface %s on path ", AUDIO_CONTROL_INTERFACE);   // log checker (BT)
