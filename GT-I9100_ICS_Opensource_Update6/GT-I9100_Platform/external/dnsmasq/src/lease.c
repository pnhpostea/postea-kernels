...
#ifdef NA_HOTSPOT_FRW
extern int dhcp_ack_finished;
//[ NAGSM_ANDROID_SEL_WIFI_Abhinav_20110801
#ifdef MAX_AP_STA_NOTI
...
void lease_update_file(time_t now)
{
  ...
  if (file_dirty != 0 && daemon->lease_stream)
	...
	file_dirty = 0;
#ifdef VZW_LTE
#ifdef GLOBALCONFIG_WLAN_SEC_MOBILEAP
...
//SISO Data Call Changes S
#ifdef NA_HOTSPOT_FRW
    if(dhcp_ack_finished == 1){
        system("ndc softap sta_join event");
		//[ NAGSM_ANDROID_SEL_WIFI_Abhinav_20110801
#ifdef MAX_AP_STA_NOTI
		char cmd[80];
		sprintf(cmd, "ndc softap sta_join event %s %s %ld", mac_addr, ip_addr, now);
        system(cmd);
#endif
		//] NAGSM_ANDROID_SEL_WIFI_Abhinav_20110801
        dhcp_ack_finished = 0;
