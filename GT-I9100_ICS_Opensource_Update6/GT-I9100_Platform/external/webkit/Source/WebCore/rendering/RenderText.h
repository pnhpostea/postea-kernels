...
// Samsung Adding for multicolumn text selection - Begin
    virtual IntRect localCaretRect(InlineBox*, int caretOffset, int* extraWidthToEndOfLine = 0,bool bMultiColumn=false);
// Samsung Adding for multicolumn text selection - End 
	...
	//SISO BIDI    
	bool checkIsRTL();
	bool applyRTL(String text);
	String handleArabicString(String str); 
	bool isRtl();		
