...
IntRect RenderObject::localCaretRect(InlineBox*, int, int* extraWidthToEndOfLine, /*SAMSUNG CHANGE*/bool bMultiColumn/*SAMSUNG CHANGE*/)
{
...
bool RenderObject::GetMultiColumnInfo(Node* pNode)
{
	...
	while (node)
	{
		RenderObject*  m_renderer = node->renderer();

		if (m_renderer->hasColumns())
		{
		  bHasColumn = true;
		  break;
		}
